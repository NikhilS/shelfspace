...
import { extractBooksFromImage, generateLibraryRecommendations, generateBookInsights, enrichBooksMetadata, extractBooksFromCsv, handleGeminiError } from './gemini';
...
describe('gemini service', () => {
  ...
  describe('handleGeminiError', () => {
    it('throws quota error for 429 status', () => {
      expect(() => handleGeminiError({ status: 429 })).toThrow('The AI service has exceeded its quota limit. Please try again later.');
    });
  });
  ...
  describe('extractBooksFromImage', () => {
    ...
    it('handles gemini error format parsing', async () => {
      ...
      await expect(extractBooksFromImage('base64', 'image/jpeg')).rejects.toThrow();
    });
  });
  ...
  describe('extractBooksFromCsv', () => {
    ...
    it('rejects on error', async () => {
      mockGenerateContent.mockRejectedValueOnce(new Error('Network error'));
      await expect(extractBooksFromCsv('title,author\nDune,Frank Herbert')).rejects.toThrow('Failed to communicate with the AI service. Please try again.');
    });
  ...
  describe('generateBookInsights', () => {
    ...
    it('rejects on error', async () => {
      ...
      await expect(generateBookInsights('Title', 'Author', 'summary')).rejects.toThrow('Failed to communicate with the AI service. Please try again.');
    });
  });
  ...
  describe('enrichBooksMetadata', () => {
    it('returns enriched metadata Array', async () => {
      const mockResult = [
        { id: '1', genre: 'Classic', series: 'Standalone' }
      ];
      mockGenerateContent.mockResolvedValueOnce({
        text: JSON.stringify(mockResult)
      });
      ...
      expect(result[0].genre).toBe('Classic');
    });

    it('rejects on error', async () => {
      mockGenerateContent.mockRejectedValueOnce(new Error('API error'));
      const input = [{ id: '1', title: '1984', author: 'George Orwell' }];
      await expect(enrichBooksMetadata(input)).rejects.toThrow('Failed to communicate with the AI service. Please try again.');
    });
  });
  ...
  describe('generateLibraryRecommendations', () => {
    ...
    it('handles gemini errors', async () => {
      mockGenerateContent.mockRejectedValueOnce(new Error('Network error'));
      await expect(generateLibraryRecommendations([{ title: 'Book 1', author: 'Author' }])).rejects.toThrow();
    });
  });
  ...
  describe('generateBookInsights', () => {
    it('returns insights for all types', async () => {
      ...
      for (const type of types) {
        const result = await generateBookInsights('1984', 'George Orwell', type);
    ...
    it('handles gemini errors', async () => {
      mockGenerateContent.mockRejectedValueOnce(new Error('Network error'));
      await expect(generateBookInsights('1984', 'George Orwell', 'summary')).rejects.toThrow();
    });
  });
