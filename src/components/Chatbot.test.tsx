...
describe('Chatbot component', () => {
  ...
  it('renders chat button initially and opens chat when clicked', async () => {
    ...
    expect(
      screen.getByText(/Hello! I'm your AI Librarian/i),
    ).toBeInTheDocument();
  });
  ...
  it('allows user to send a message and renders stream response', async () => {
    const user = userEvent.setup();
    render(
      <Chatbot libraryBooks={[{title: 'Dune', author: 'Frank Herbert'}]} />,
    );

    // Open chat
    await user.click(
      screen.getByRole('button', {name: /Open AI Librarian Chat/i}),
    );
    ...
    mockSendMessageStream.mockResolvedValueOnce(mockStream);
