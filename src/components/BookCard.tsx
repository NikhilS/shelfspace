...
export default function BookCard({ book, onDelete, onClick, canEdit, compact = false }: BookCardProps) {
  ...
  return (
    <div 
      onClick={onClick}
      className={`group relative ${compact ? 'w-8 h-48 sm:w-10 sm:h-56' : 'w-32 h-48 sm:w-40 sm:h-56'} rounded-r-xl rounded-l-sm shadow-md transition-all duration-400 ease-out hover:-translate-y-4 hover:shadow-2xl hover:shadow-accent/20 hover:rotate-1 cursor-pointer flex-shrink-0 ring-1 ring-black/5 hover:ring-accent/40`}
    >
      {/* Book Spine Effect */}
      ...
      {/* Book Cover / Spine */}
      <div 
        className={`w-full h-full rounded-r-xl rounded-l-sm overflow-hidden relative ${(!book.coverUrl || compact) ? `bg-gradient-to-br ${gradientClass}` : 'bg-surface'}`}
      >
        {compact ? (
          <div className="absolute inset-0 flex items-center justify-center py-4 px-1 text-ink/80">
            <h3 
              className="font-serif font-bold text-xs sm:text-sm tracking-widest whitespace-nowrap overflow-hidden text-ellipsis drop-shadow-sm"
