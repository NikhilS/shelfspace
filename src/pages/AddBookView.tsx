...
import { Search, Camera, X, BookPlus, Loader2, UploadCloud, FileText, Plus, Sparkles, ArrowLeft, ScanBarcode, ChevronDown } from 'lucide-react';
...
export default function AddBookView() {
  ...
  const [activeTab, setActiveTab] = useState<'scan' | 'search' | 'camera' | 'csv' | 'manual'>('scan');
  ...
  const onAddBook = async (book: BookDetails) => {
    ...
    const cleanDetails = Object.fromEntries(
      Object.entries(enrichedDetails).filter(([_, v]) => v !== undefined && v !== null && v !== '')
    );
    if (cleanDetails.genre && typeof cleanDetails.genre === 'string') cleanDetails.genre = cleanDetails.genre.substring(0, 100);
    if (cleanDetails.author && typeof cleanDetails.author === 'string') cleanDetails.author = cleanDetails.author.substring(0, 500);
    if (cleanDetails.series && typeof cleanDetails.series === 'string') cleanDetails.series = cleanDetails.series.substring(0, 100);
    if (cleanDetails.title && typeof cleanDetails.title === 'string') cleanDetails.title = cleanDetails.title.substring(0, 500);
  ...
  const handleScanIsbn = async (isbn: string) => {
    if (processingIsbns.has(isbn) || scannedBooks.some(b => b.isbn === isbn)) return;
  ...
  const handleAddSelectedScanned = async () => {
    setIsAddingAll(true);
    let successCount = 0;
    const booksToAdd = scannedBooks.filter(b => selectedScanned.has(b.isbn || b.title));
    ...
    toast.success(`Successfully added ${successCount} out of ${booksToAdd.length} books`);
    setScannedBooks(prev => prev.filter(b => !selectedScanned.has(b.isbn || b.title)));
      ...
      const batchPromises = batch.map(async (book) => {
        try {
          ...
          if (!bookToAdd) {
            ...
            bookToAdd = book.author ? (results.find(r => (r.author || '').toLowerCase().includes(book.author.toLowerCase())) || results[0] || null) : (results[0] || null);
          }
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-8 lg:px-12 py-6 sm:py-8 min-w-0 max-w-[1200px] mx-auto w-full">
        ...
        <div className="bg-surface rounded-2xl sm:rounded-[32px] w-full flex flex-col overflow-hidden shadow-sm border border-outline-variant/30 relative">
          <div className="px-4 sm:px-6 py-4 sm:py-5 bg-surface-container-lowest border-b border-outline-variant/30 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            <span className="font-bold text-sm text-on-surface mb-1 sm:mb-0">Selected Method:</span>
            <div className="relative w-full sm:w-64" ref={dropdownRef}>
              ...
              >
                <div className="flex items-center gap-2 font-bold text-sm text-on-surface">
                  {activeTab === 'scan' && <><ScanBarcode size={18} className="text-primary" /> Scan ISBN</>}
                  {activeTab === 'camera' && <><Camera size={18} className="text-primary" /> Capture Shelf</>}
                  {activeTab === 'csv' && <><FileText size={18} className="text-primary" /> Import CSV</>}
                  {activeTab === 'search' && <><Search size={18} className="text-primary" /> Search Database</>}
                  {activeTab === 'manual' && <><Plus size={18} className="text-primary" /> Manual Entry</>}
                </div>
                <ChevronDown size={16} className={`text-outline transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
                    ...
                    {[
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (tab.id === 'camera' || tab.id === 'scan') stopCamera(); // Will be started appropriately by other effects if needed
                          else stopCamera();
                      ...
                      >
                        <tab.icon size={18} className={activeTab === tab.id ? 'text-primary' : 'text-outline'} />
                        {tab.label}
                      </button>
                    ))}
                  </motion.div>
              ...
              <div className="space-y-6 flex flex-col items-center">
                ...
                {processingIsbns.size > 0 && (
                  <div className="flex items-center gap-2 text-primary bg-primary/10 px-4 py-2 rounded-full">
                    ...
                    <span className="font-medium text-sm">Searching for {processingIsbns.size} ISBN{processingIsbns.size > 1 ? 's' : ''}...</span>
                  </div>
                )}
                ...
                {scannedBooks.length > 0 && (
                  <div className="w-full space-y-4">
                    <div className="flex items-center justify-between sticky top-0 bg-surface/80 backdrop-blur-xl py-3 px-2 z-10 border-b border-outline-variant/40 mb-2 rounded-t-xl -mx-2">
                      <h3 className="font-serif text-xl sm:text-2xl font-bold text-on-surface tracking-tight">Scanned {scannedBooks.length} Books</h3>
                      <div className="flex gap-2 sm:gap-3 items-center">
                        <label className="hidden sm:flex items-center gap-2 text-sm font-bold text-on-surface cursor-pointer mr-2 hover:bg-surface-container-low/80 px-3 py-1.5 rounded-full transition-colors">
                          <input 
                            type="checkbox" 
                            checked={selectedScanned.size === scannedBooks.length && scannedBooks.length > 0} 
                            onChange={(e) => {
                              if (e.target.checked) setSelectedScanned(new Set(scannedBooks.map(b => b.isbn || b.title)));
                        ...
                        <button 
                          onClick={() => { setScannedBooks([]); setSelectedScanned(new Set()); }}
                        ...
                        >
                          ...
                          <span className="hidden sm:inline">Add Selected </span>({selectedScanned.size})
                        </button>
                      </div>
                    </div>
